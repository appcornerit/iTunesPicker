...
@interface ITPPickerDetailViewController ()<SwipeViewDataSource, UITableViewDataSource, UITableViewDelegate, ITPViewControllerDelegate>
{
...
@implementation ITPPickerDetailViewController
...
- (void)viewDidLoad
{
    ...
    [self.purchaseButton setTitle:NSLocalizedString(@"itppickerdetail.button.purchase",nil) forState:UIControlStateNormal];    
    ...
    self.ratingsAllVersionsLabel.text = [NSString stringWithFormat:@"(%@ %@)", [decimalFormatter stringFromNumber:userRatingCountAllVersions],NSLocalizedString(@"itppicker.table.label.allVersions",nil)];
...
- (IBAction)openiTunesStore:(id)sender
{
    ...
    [query openEntity:self.appObject inITunesStoreCountry:self.delegate.entitiesDatasources.userCountry completionBlock:^(BOOL succeeded, NSError *err) {
        if(!succeeded || err)
        {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (indexPath.row) {
        case 0:
            ...
            cell.detailTextLabel.text = NSLocalizedString(@"itppickerdetail.label.developer",nil);
            cell.detailTextLabel.textColor = [UIColor blueColor];
            break;
        case 1:
            cell.textLabel.text = self.appObject.sellerName;
            cell.detailTextLabel.text = NSLocalizedString(@"itppickerdetail.label.seller",nil);
            break;
        case 2:
            cell.textLabel.text = [dateFormatter stringFromDate:self.appObject.releaseDate];
            cell.detailTextLabel.text = NSLocalizedString(@"itppickerdetail.label.update",nil);
            break;
        case 3:
            cell.textLabel.text = self.appObject.version;
            cell.detailTextLabel.text = NSLocalizedString(@"itppickerdetail.label.version",nil);
            break;
        case 4:
            cell.textLabel.text = [NSString stringWithFormat:@"%d MB",(int)round([self.appObject.fileSizeBytes doubleValue]/(1024*1024))];
            cell.detailTextLabel.text = NSLocalizedString(@"itppickerdetail.label.size",nil);
            break;
        case 5:
            cell.textLabel.text = self.appObject.contentAdvisoryRating;
            cell.detailTextLabel.text = NSLocalizedString(@"itppickerdetail.label.rating",nil);
            break;
        case 6:
            cell.textLabel.text = [self.appObject.languageCodesISO2A componentsJoinedByString:@","];
            cell.detailTextLabel.text = NSLocalizedString(@"itppickerdetail.label.languages",nil);
            break;
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(indexPath.row == 0)
    {
        ...
        [pickerTableView loadEntitiesForArtistId:self.appObject.artistId inAppStoreCountry:self.pickerCountry withType:self.appObject.iTunesEntityType completionBlock:^(NSArray *array, NSError *err) {
